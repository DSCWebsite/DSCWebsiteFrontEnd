...
const AboutusDepartment = () => {
  ...
  return (
    <motion.section
      ...
      className={`bg-[#00031E] lg:min-h-screen w-screen flex flex-col align-middle mt-10 pt-10 lg:p-14 ${inter500.className}`}
    >
      <motion.h1
        initial="hidden"
        whileInView="show"
...

          {
            listDepartment.map((department) => <Link href={`/aboutus/${department.slug}`}>
              <Image
                ...
                width={300}
                height={300}
                className="lg:block bg-white hover:-translate-y-3 transition-all hover:cursor-pointer hover:shadow-white border rounded-md"
              />
            </Link>)
          }
        </div>
